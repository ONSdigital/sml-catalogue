...
def navigate_to_url(context, page):
    if page == "methods catalogue":
        main_navigation = WebDriverWait(driver, timeout=timeout).until(
            lambda d: d.find_element(By.ID, value="main-navigation")
        )
        ...
        main_navigation = WebDriverWait(driver, timeout=10).until(
            lambda d: d.find_element(By.ID, value="main-navigation")
        )
