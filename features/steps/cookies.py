...
def click_dropdown(context):
    ...
    WebDriverWait(driver, timeout=10).until(EC.presence_of_element_located((By.ID, 'cookies-essential-details')))

@when('I change my settings to enable cookies')
def enable_cookie(context):
    WebDriverWait(driver, timeout=10).until(lambda d: d.find_element(By.ID, "on-3-label")).click()
...
def disable_cookie(context):
    WebDriverWait(driver, timeout=10).until(lambda d: d.find_element(By.ID, "off-3-label")).click()
    WebDriverWait(driver, timeout=10).until(lambda d: d.find_element(By.ID, "ons_cookie_save_changes")).click()
    WebDriverWait(driver, timeout=10).until(EC.presence_of_element_located((By.ID, 'alert')))
...
def cookie_enabled_check(context):
    alert_content = WebDriverWait(driver, timeout=40).until(lambda d: d.find_element(By.ID, "cookie_change_alert")).text
    print(alert_content)
