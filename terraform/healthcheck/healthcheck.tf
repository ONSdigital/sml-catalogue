...
resource "aws_cloudwatch_event_target" "sml_site_trigger_healthcheck" {
    ...
    input     = jsonencode({
                  "site"            = "https://${var.domain_name_base[var.environment]}",
                  "env"             = "${var.environment}"
...
resource "aws_lambda_function" "healthcheck" {
  ...
  environment {
    variables = {
      "expected_string" = "An open source library for statistical code approved by the ONS",
      "env" = var.environment,
      "site" = var.domain_name_base[var.environment],
    }
  }
...
resource "aws_cloudwatch_metric_alarm" "healthcheck" {
  ...
  statistic           = "Sum"
  threshold           = 3
  alarm_description   = "Alarm for ${var.domain_name_base[var.environment]} has been triggered"
